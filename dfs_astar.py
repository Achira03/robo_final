...
shot_targets = set()
last_fire_time = 0
# ✅✅✅ เพิ่ม: ตัวแปร Global สำหรับเก็บตำแหน่ง Grid ปัจจุบันของหุ่นยนต์
robot_current_grid_pos = (0, 0)

# ===================================================================
# SECTION 2: MERGED CALLBACKS & HELPER FUNCTIONS
# ===================================================================
# ... (โค้ดส่วนนี้ทั้งหมดเหมือนเดิม ไม่มีการเปลี่ยนแปลง) ...
...
def detection_and_shooting_thread_func(ep_robot, current_pos_getter):
    ...
    while program_running:
        ...
        frame = img.copy()
        # ... (ส่วนประมวลผลภาพเหมือนเดิมทั้งหมด) ...
            ...
            for cnt in contours:
                ...
                if area > 500:
                    ...
                    if shape_name != "Unknown":
                        ...
                        if area > biggest_valid_area and not any(t[0] == target_id for t in shot_targets):
                            biggest_valid_area = area
            ...
            if gimbal_lock.acquire(blocking=False):
                try:
                    ...
                    if is_in_range and is_centered and is_cooldown_passed:
                        ...
                        print(f"[Detection Thread] Target {target_color} {target_shape} at {current_robot_pos} centered. FIRING!")
                        ep_blaster.fire(fire_type=blaster.WATER_FIRE, times=3)
                        shot_targets.add((aiming_target_info["id"], current_robot_pos))
...
# ===================================================================
# SECTION 4: MOVEMENT & MAZE FUNCTIONS (Unchanged)
# ===================================================================
# ... (โค้ดส่วนนี้ทั้งหมดเหมือนเดิม ไม่มีการเปลี่ยนแปลง) ...
...
def run_exploration_mode(ep_robot):
    global program_running, robot_current_grid_pos
    ...
    current_pos = (0, 0)
    robot_current_grid_pos = current_pos # ✅✅✅ อัปเดตตำแหน่งเริ่มต้น
    ...
    while path_stack and program_running:
        ...
        robot_current_grid_pos = current_pos # ✅✅✅ อัปเดตตำแหน่งทุกครั้งที่เริ่ม Loop
        cx, cy = current_pos
        print(f"\n=================================================")
        ...
        # ... (ส่วน Logic การสร้างแผนที่และการหาเส้นทาง เหมือนเดิม) ...
    ...
    for target_id, pos in shot_targets:
        if target_id not in targets_found_at_coords:
            targets_found_at_coords[target_id] = pos
    ...
    for target_id, pos in targets_found_at_coords.items():
        print(f"  - {target_id} at position {pos}")
    
    save_run_data(maze_map, targets_found_at_coords, DATA_FILENAME)

def run_optimal_path_mode(ep_robot):
    global shot_targets, last_fire_time, robot_current_grid_pos
    ep_chassis = ep_robot.chassis
    ep_gimbal = ep_robot.gimbal
    ...
    start_node = (0, 0)
    robot_current_grid_pos = start_node # ✅✅✅ อัปเดตตำแหน่งเริ่มต้น
    target_locations = list(targets_found.values())
    ...
    if best_path_order:
        for i in range(len(best_path_order) - 1):
            ...
            for j in range(len(segment_path) - 1):
                ...
                robot_current_grid_pos = current_step # ✅✅✅ อัปเดตตำแหน่งก่อนจะเลี้ยว
            ...
            robot_current_grid_pos = end_segment # ✅✅✅ อัปเดตตำแหน่งเมื่อถึงปลายทางของ Segment
            print(f"\nArrived at target location {end_segment}. Re-acquiring and firing...")
            robot_is_stationary.set()
            time.sleep(5) 
            robot_is_stationary.clear()
